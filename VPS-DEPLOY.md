...
### 1.1 — IP السيرفر
سيرفركِ: **`66.29.143.196`** (AlmaLinux 9). استخدمي هذا الرقم في كل مكان أدناه.
...
| Type | Host | Value | TTL |
|---|---|---|---|
| **A Record** | `@` | `66.29.143.196` | Automatic |
| **A Record** | `www` | `66.29.143.196` | Automatic |

> نفس الـ IP في السجلّين.
...
### 2.1 — افتحي السيرفر (طريقتان)

**الطريقة A — VNC من لوحة التحكّم (الأسهل، بدون برامج)**:
في لوحة VPS Control اضغطي زرّ **VNC** → تفتح شاشة السيرفر في المتصفّح →
سجّلي دخول بـ `root` وكلمة السرّ.

**الطريقة B — SSH من جهازكِ**:
افتحي **PowerShell** على Windows واكتبي:
```bash
ssh root@66.29.143.196
```
ستُطلب كلمة السرّ (الجديدة التي غيّرتِها).

### 2.2 — ثبّتي git واجلبي المشروع
سيرفركِ AlmaLinux، لذا نستخدم `dnf`. الصقي:
