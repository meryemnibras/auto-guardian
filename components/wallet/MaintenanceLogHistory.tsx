  ...
  return (
    <div className="space-y-2 rounded-2xl border border-slate-200 dark:border-gray-800 bg-white dark:bg-gray-900/60 p-3">
      ...
      {loading && entries.length === 0 && (
        <div className="flex items-center justify-center py-4 text-xs text-slate-600 dark:text-gray-500">
          <Loader2 className="h-4 w-4 animate-spin" aria-hidden />
        </div>
      )}

      {!loading && entries.length === 0 && (
        <div className="rounded-xl border border-dashed border-slate-200 dark:border-gray-800 px-3 py-4 text-center text-xs text-slate-600 dark:text-gray-500">
          {t("noServiceLogged")}
        </div>
      )}
      ...
      <ul className="space-y-1.5">
        {entries.map((e) => {
          ...
          return (
            <li
              key={e.id}
              className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-gray-800 bg-gray-950/50 px-3 py-2 text-xs"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[10px] text-slate-600 dark:text-gray-500">
              ...
              <button
                ...
                className="rounded-lg p-1 text-slate-600 dark:text-gray-500 transition-colors hover:bg-rose-500/10 hover:text-rose-300 active:scale-95"
              >
