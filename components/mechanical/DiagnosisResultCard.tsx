...
export function DiagnosisResultCard({ result, saved }: DiagnosisResultCardProps) {
  ...
  return (
    <motion.article
      ...
      className={`relative overflow-hidden rounded-2xl border border-gray-800 bg-gradient-to-br ${style.bg} bg-gray-900/80 p-5 ring-1 ${style.ring} shadow-lg`}
    >
      <div className="flex items-start gap-3">
        <div className={`shrink-0 rounded-xl bg-gray-950/60 p-2 ${style.icon}`}>
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="rounded-xl border border-gray-800 bg-gray-950/50 p-3">
            ...
            <p className="mt-1 text-sm text-gray-200">{result.recommendation}</p>
          </div>
          {saved !== null && (
            <p
              className={`text-[11px] ${
